...
import { UpdatePostByIdUseCase } from "./update-post-by-id.use-case";
...
describe("UpdatePostByIdUseCase", () => {
	...
	beforeEach(async () => {
		...
		useCase = new UpdatePostByIdUseCase(
			postRepository,
	...
	it("should update basic fields successfully", async () => {
		...
		const result = await useCase.run(existingPost.id, updateDto);
		...
		// Verify persistence
		const persisted = await postRepository.findById(existingPost.id);
	...
	it("should update name and slug when name changes significantly", async () => {
		...
		const oldSlugSearch = await postRepository.findBySlug(existingPost.slug); // existingPost.slug is "original-post"
	...
	it("should throw ResourceNotFoundException if post does not exist", async () => {
		const promise = useCase.run(generateUUID(), {});
	...
	it("should throw ResourceAlreadyExistsException if new name generates an existing slug", async () => {
		...
		const promise = useCase.run(existingPost.id, updateDto);
